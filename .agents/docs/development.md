...
What to type to set a live check up and follow it. What makes one, and when to run it, is in `.agents/docs/testing.md`.
...
- `GITHUB_TOKEN` — authorizes pull-request reads, review comments, main-push issues, and the bounded duplicate check. Required by `coral resolve` and `coral publish`, and by `coral review` only for a staged main-push review; it never reaches an agent container. Each job's token is scoped by that job's `permissions` block in the reusable workflow and expires when the job ends. Locally, `gh auth token` supplies one.
...
- Coral is installed on this repository by `.github/workflows/review.yml`, which calls the workflow beside it rather than a pinned reference. Every trigger reads that caller from the default branch; use `coral rehearse` or `kkestell/coral-test` to exercise a Coral change before merging it.
