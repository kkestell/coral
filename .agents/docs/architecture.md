...
- Review builds the agent, runs it, and posts the result.
...
- `coral/review.py` — render the request, run the agent, post the result.
...
- `coral/prompts/review.md` — what Coral looks for. A placeholder; the content is item 6.
...
- The agent's only return value is the structured review object, and deterministic code does the posting. Nothing the model produces becomes a push, an approval, or a comment Coral did not compose.
...
- The agent reaches the checkout only through the backend. Coral's own code runs `git` there once, in `coral/diff.py`, so the diff the agent saw and the diff the anchors are checked against are the same.
