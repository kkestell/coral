...
- The model profile is supplied by hand through `profile=` rather than left to the lookup, so it stays a decision `coral/agent.py` makes. A profile-less model gets summarization triggers scaled to 170,000 tokens instead of the real million, and one key states that this model takes no `temperature`.
