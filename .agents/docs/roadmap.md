...
Get the whole workflow running end to end with no model call: `coral review` returns one hardcoded summary and one hardcoded finding on a line picked from the diff. Everything around it is real — the composite actions, the reusable workflow, the `$/` references, the reaction, the sentinel, the batched review. `kkestell/coral-test` gets its first pull request here.
...
- A comment event runs in the base repository with its token and secrets, so resolve stops when the head repository differs from the base, a deleted head repository counting as a fork. The `pull_request` path is filtered too, though GitHub already withholds secrets there.
...
- Both write permissions are needed: reactions on diff comments go through `/pulls/comments/{id}/reactions` (Pull requests permission), on whole-pull-request comments through `/issues/comments/{id}/reactions` (Issues permission); neither grants the other.
...
- Neither secret reaches the agent's shell, twice over: the shell environment is built rather than inherited and names neither; and the review step reads both from `os.environ` at start-up, holds them in memory, deletes them, and only then constructs the model client and backend. Without this, `pull-requests: write` in the environment is an approving review one `curl` away.
...
- A fired deadline raises out of `invoke` rather than ending the run gracefully: a graceful end would arrive as "the agent returned no structured review" with the reason lost. Until item 8 exists that is a red run.
...
- A finding with a test is confirmed only if that test, written out from the finding's own content, fails for the reason claimed; a speculative one only if the claimed behavior is in the source.
