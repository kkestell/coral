"""The container every tool the agent holds runs in, and the only place Coral speaks to `docker`.
...
# Pinned by digest because a tag moves under whoever reads it. This is the multi-architecture index
# for `ubuntu:24.04`, read 2026-08-07.
...
def docker(arguments: list[str]) -> None:
    ...
    # The message reaches the CLI when a review fails; `CalledProcessError` carries only the status.
